...
template <typename T> int solve(T array, int left, int right, int k) {
    if (left > right) return -1;
    int mid = (left + right) / 2;
    if (array[left] > array[right]) std::swap(array[left], array[right]);
    if (array[right] < array[mid]) std::swap(array[right], array[mid]);
...
template <typename T> auto findKth(T array, int n, int k) {
